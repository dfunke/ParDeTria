...
#ifndef CGAL_Indexed_Triangulation_data_structure_2_H
#define CGAL_Indexed_Triangulation_data_structure_2_H
...
namespace CGAL {
    ...
    class Indexed_Triangulation_data_structure_2
            : public Triangulation_cw_ccw_2 {
        typedef Indexed_Triangulation_data_structure_2<Vb, Fb, Ih> Tds;
    ...
    public:
        ...
        struct Rebind_vertex {
            typedef Indexed_Triangulation_data_structure_2<Vb2, Fb, Ih> Other;
        };

        template<typename Fb2>
        struct Rebind_face {
            typedef Indexed_Triangulation_data_structure_2<Vb, Fb2, Ih> Other;
        };
    ...
    public:
        Indexed_Triangulation_data_structure_2();

        Indexed_Triangulation_data_structure_2(const Tds &tds);
...
// 		  FaceIt face_end);
    ...
    template<class Vb, class Fb, class Ih>
    Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::
    Indexed_Triangulation_data_structure_2()
            : _dimension(-2) { }
...
//ACCESS FUNCTIONS
    template<class Vb, class Fb, class Ih>
    inline
    typename Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::size_type
    Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::
...
// with face fr on the right of a->b
    ...
    template<class Vb, class Fb, class Ih>
    typename Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::Vertex_handle
    Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::
    insert_first() {
    ...
    //insert in the edge opposite to vertex i of face f
    {
        ...
        if (dimension() == 2) {
            CGAL_triangulation_precondition(i == 0 ||
                                            i == 1 ||
                                            i == 2);
        }
        Vertex_handle v;
        ...
        switch (dim) {
            ...
            case 2 : {
                ...
                for (; lfit != to_delete.end(); ++lfit) {
                    ...
                    f1 = f->neighbor(dim);
                    i1 = mirror_index(f, dim); //f1->index(f);
                    f2 = f->neighbor(j);
                    i2 = mirror_index(f, j); //f2->index(f);
...
//           (dimension() == 2 && number_of_vertices() > 3) );
    ...
    template<class Vb, class Fb, class Ih>
    typename Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::Face_handle
    Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::
    create_face() {
        Face_handle newf = faces().emplace(_indexHandler);
        return newf;
    }
    ...
    create_face(Vertex_handle v1, Vertex_handle v2, Vertex_handle v3) {
        Face_handle newf = faces().emplace(v1, v2, v3, _indexHandler);
        return newf;
    }
    ...
    create_face(Vertex_handle v1, Vertex_handle v2, Vertex_handle v3,
        ...
        Face_handle newf = faces().emplace(v1, v2, v3, f1, f2, f3, _indexHandler);
...
// split and join operations
    ...
    template<class Vb, class Fb, class Ih>
    typename Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::Fourtuple
    Indexed_Triangulation_data_structure_2<Vb, Fb, Ih>::
...
// CHECKING
    ...
    operator>>(std::istream &is,
               Indexed_Triangulation_data_structure_2<Vb, Fb, Ih> &tds) {
        tds.file_input(is);
        return is;
    }
    ...
    operator<<(std::ostream &os,
               const Indexed_Triangulation_data_structure_2<Vb, Fb, Ih> &tds) {
        tds.file_output(os);
        return os;
    }
...
#endif //CGAL_Indexed_Triangulation_data_structure_2_H
